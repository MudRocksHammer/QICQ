...
void MyApp::ReadSettingFile(){
    ...
    settings.beginGroup("Server");
    m_strHostAddr = settings.value("HostAddr", "192.168.1.2").toString();
...
void MyApp::CheckSound(){
    if(!QFile::exists(MyApp::m_strSoundPath + "message.wav")){
        QFile::copy(":/resource/sound/message.wav", MyApp::m_strSoundPath + "message.wav");
    }

    if (!QFile::exists(MyApp::m_strSoundPath + "msg.wav")) {
        QFile::copy(":/resource/sound/msg.wav", MyApp::m_strSoundPath + "msg.wav");
    }

    if (!QFile::exists(MyApp::m_strSoundPath + "ringin.wav")) {
        QFile::copy(":/resource/sound/ringin.wav", MyApp::m_strSoundPath + "ringin.wav");
    }

    if (!QFile::exists(MyApp::m_strSoundPath + "system.wav")) {
        QFile::copy(":/resource/sound/system.wav", MyApp::m_strSoundPath + "system.wav");
    }

    if (!QFile::exists(MyApp::m_strSoundPath + "userlogon.wav")) {
        QFile::copy(":/resource/sound/userlogon.wav", MyApp::m_strSoundPath + "userlogon.wav");
    }
}
...
void MyApp::SaveConfig(){
    ...
    settings.setValue("HostAddr", m_strHostAddr);
    settings.setValue("MsgPort", m_nMsgPort);
    settings.setValue("FilrPort", m_nFilePort);
    settings.setValue("GroupPort", m_nGroupPort);
    settings.endGroup();
    settings.sync();
}
